...
static esp_err_t on_default_url(httpd_req_t *req)
{
  ...
  //char pages[4] = {".local/ap-to-sta-backup" , ".local/ap-to-sta" , ".local/index" , ".local/toggle-led"};
...
// static esp_err_t on_show_time_url(httpd_req_t *req)
// {
//   char buffer[100];
//   memset(&buffer, 0, sizeof(buffer));
//   httpd_req_recv(req, buffer, req->content_len);
//   cJSON *payload = cJSON_Parse(buffer);
//   cJSON *time_str = cJSON_GetObjectItem(payload, "proj_time");
//   bool is_on = cJSON_IsTrue(time_str);
//   cJSON_Delete(payload);
//   toggle_led(is_on);
//   httpd_resp_set_status(req, "204 NO CONTENT");
//   httpd_resp_send(req, NULL, 0);
...
void init_server()
{
  ...
  // httpd_uri_t show_time_url = {
  //     .uri = "/api/show-time",
  //     .method = HTTP_GET,
  //     .handler = on_show_time_url};
  // httpd_register_uri_handler(server, &show_time_url);
